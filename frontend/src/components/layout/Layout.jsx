...
export function TopBar() {
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200/90 sticky top-0 z-30 flex items-center justify-between px-4 md:px-6 select-none font-sans shadow-2xs">
      ...
      <div className="flex items-center space-x-3.5">
        ...
        >
          <div className="w-9 h-9 rounded-xl bg-slate-900 text-white flex items-center justify-center font-bold text-sm shadow-xs group-hover:bg-blue-600 transition-colors">
      ...
      <div className="hidden lg:flex items-center flex-1 max-w-md mx-6 relative">
        <div className="relative w-full">
          ...
          <input 
            ...
            className="w-full bg-slate-50 hover:bg-slate-100/70 focus:bg-white border border-slate-200 rounded-full pl-10 pr-4 py-2 text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-blue-500 transition-colors"
        ...
        {isSearchOpen && globalSearchResults && (
          <div className="absolute top-12 left-0 right-0 bg-white border border-slate-200 rounded-2xl shadow-lg p-2 z-50 space-y-1 max-h-80 overflow-y-auto">
            <div className="flex items-center justify-between px-3 py-1.5 border-b border-slate-100">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Search Records</span>
              <button onClick={() => setIsSearchOpen(false)} className="p-1 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100">
            ...
            {globalSearchResults.length === 0 ? (
              ...
              globalSearchResults.map((res, i) => (
                <div 
                  ...
                  className="p-2.5 hover:bg-slate-50 rounded-xl cursor-pointer flex items-center justify-between transition-colors"
                >
                  <div>
                    <div className="font-semibold text-slate-900 text-xs">{res.title}</div>
                    <div className="text-[11px] text-slate-500">{res.subtitle}</div>
                  </div>
                  <span className="text-[10px] font-medium px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-slate-200 uppercase">
                    {res.type}
                  </span>
                </div>
              ))
      ...
      <div className="flex items-center space-x-2.5">
        ...
        {/* Role Selector */}
        <div className="hidden md:flex items-center bg-slate-100 p-1 rounded-full border border-slate-200 text-xs">
        ...
        <div className="flex items-center space-x-1.5 pl-1">
          <div className="w-8 h-8 rounded-full bg-slate-900 text-white flex items-center justify-center font-bold text-xs" title={`${currentUser.name} (${currentUser.role})`}>
            {currentUser.name ? currentUser.name.charAt(0) : 'U'}
          </div>

          <button 
            onClick={logout}
            className="p-2 text-slate-400 hover:text-rose-600 hover:bg-slate-100 rounded-full transition-colors"
            title="Sign Out"
          >
    ...
    return (
      <div className="p-3 space-y-4 flex-1 overflow-y-auto overflow-x-hidden font-sans text-xs flex flex-col justify-between">
              ...
              return (
                <button 
                  ...
                  className={`w-full flex items-center transition-all duration-150 relative ${
                    showFull ? 'justify-between px-3 py-2 rounded-xl' : 'justify-center p-2.5 rounded-xl'
                  } font-medium group ${
                    isActive 
                      ? 'bg-slate-900 text-white font-semibold shadow-xs border border-slate-800' 
                ...
                >
                  <div className="flex items-center space-x-2.5 min-w-0">
                    <div className={`p-1 rounded-lg transition-colors shrink-0 ${
                      isActive 
                        ...
                        : 'text-slate-500 group-hover:text-slate-800 group-hover:bg-slate-200/50'
                    }`}>
                  ...
                  {showFull && (
                    <div className="flex items-center space-x-1.5 shrink-0 pl-1">
                      {item.isLive && (
                        <span className={`inline-flex items-center space-x-1 px-2 py-0.5 rounded-full text-[9px] font-bold tracking-wider uppercase transition-colors ${
                          isActive
                      ...
                      {item.adminOnly && (
                        <span className={`text-[9px] font-bold tracking-wider px-2 py-0.5 rounded-full uppercase transition-colors ${
                          isActive 
                      ...
                      {!item.isLive && !item.adminOnly && item.badge && (
                        <span className={`text-[10px] font-semibold px-2 py-0.2 rounded-full transition-colors ${
                          isActive 
                            ? 'bg-white/10 text-slate-200 border border-white/10' 
                            : 'bg-slate-100 text-slate-500 border border-slate-200/80 group-hover:bg-slate-200/60'
                        }`}>
                          {item.badge}
                        </span>
                      )}
        ...
        <div className="space-y-3 pt-3 border-t border-slate-200">
          ...
          {showFull ? (
            <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-3 space-y-2.5 shadow-xs">
              {/* Header */}
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-1.5">
                  <span className="flex items-center space-x-1 text-[10px] font-semibold bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                    <span>LIVE</span>
                  </span>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-2 space-y-2">
                <div className="flex items-center justify-between gap-1.5 text-xs">
                  <div className="flex items-center space-x-1.5 min-w-0">
                    ...
                    <select
                      ...
                      className="bg-slate-50 border border-slate-200 text-slate-800 font-semibold text-xs rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500 cursor-pointer"
                    >
                  ...
                  <span className="text-[11px] text-blue-600 font-medium bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-full shrink-0">
                ...
                <div className="grid grid-cols-3 gap-1 pt-1.5 border-t border-slate-100 text-center">
                  <div className="bg-slate-50 rounded-lg py-1 px-0.5 border border-slate-100">
              ...
              <div className="grid grid-cols-2 gap-1.5 text-xs">
                <button
                  ...
                  className="py-1.5 px-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200/80 rounded-xl font-semibold text-xs flex items-center justify-center space-x-1 transition-colors cursor-pointer"
                ...
                <button
                  ...
                  className="py-1.5 px-2 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-xl font-semibold text-xs flex items-center justify-center space-x-1 transition-colors cursor-pointer"
              ...
              <button 
                ...
                className="w-full flex items-center justify-center space-x-1.5 py-2 px-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs transition-colors shadow-xs cursor-pointer"
              >
            ...
            <div className="flex justify-center relative">
              <button 
                ...
                className="w-10 h-10 rounded-2xl bg-slate-900 hover:bg-blue-600 text-white flex items-center justify-center transition-all shadow-sm relative group"
          ...
          <div className={`border border-slate-200/90 bg-slate-50/80 rounded-2xl transition-all shadow-2xs ${
            showFull ? 'p-2.5' : 'p-2 flex justify-center'
          }`}>
            <div className="flex items-center space-x-2.5">
              <div className="relative shrink-0">
                <div 
                  className="w-8 h-8 rounded-full bg-slate-900 text-white flex items-center justify-center font-bold text-xs ring-2 ring-emerald-500/30" 
              ...
              {showFull && (
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">{currentUser.role || 'Officer'}</span>
                  </div>
